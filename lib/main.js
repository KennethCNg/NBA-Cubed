...
let scene, camera, renderer, geometry, material, mesh, sphere, light, light2, lines, map, controls, raycaster, mouse, INTERSECTED;
let playerStatArr = [];
let cubeArr = [];
...
function makeCubes() {
    ...
    for ( let i = 0; i < len; i++ ) {
        ...
        radius = i;
        const cube = new playerCube(playerStatArr[i].teamName, angle, ppg, radius);
...
function animate() {
    ...
    for ( let i = 0; i < scene.children.length; i++ ) {
        let intersects = raycaster.intersectObjects( scene.children[i] );   
