...
function init() {
    ...
    grabStats();
    setInterval(grabStats, 10000);
    sortPlayersByPpg();

    // requests new data from the API every 10 seconds
    setInterval(grabStats, 10000);
    setInterval(sortPlayersByPpg, 10000);
...
function grabStats() {
    ...
    let playersJSON = JSON.parse(data.getBody('utf8')).cumulativeplayerstats.playerstatsentry;
    let len = playersJSON.length;
    let playerObj, teamName, firstName, lastName, gamesPlayed, twoPts, threePts, ftMade;

    for (let i = 0; i < len; i++) {
        if (playersJSON[i].team === null) {
            continue;
        };
